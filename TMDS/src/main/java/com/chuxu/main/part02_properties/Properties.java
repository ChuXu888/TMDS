...
public class Properties {
    ...
    //性质3：当|VV0∪VV1|=0时，若图G=(V,E)中存在|N(G,v)\V0|=r的结点v，则(N(G,v)\V0)中的所有结点一定在最优解中，此时将(N(G,v)\V0)加入V1。
    public static LinkedHashSet<Integer> property03(LinkedHashSet<Node> W, LinkedHashSet<Node> V, LinkedHashSet<Node> V1) {
        //V\V0=V1∪V5=V1∪V
        LinkedHashSet<Node> V_Add_V1 = new LinkedHashSet<>(V);
        V_Add_V1.addAll(V1);
...
//                node1.setRemainCount(0);  //进行全局属性修改
    ...
    //性质4：当|VV0∪VV1|≠0时，若图G=(V,E)中存在|N(G,v)\V0\VV0|=r的结点v，则(N(G,v)\V0\VV0)中的所有结点一定在最优解中，此时将(N(G,v)\V0\VV0)加入VV1。
    public static LinkedHashSet<Integer> property04(LinkedHashSet<Node> W, LinkedHashSet<Node> V, LinkedHashSet<Node> V1, LinkedHashSet<Node> VV1) {
...
//                node1.setRemainCount(0);  //进行全局属性修改
    ...
    //性质6：若图G=(V,E)中所有结点的度deg(G,v)均为r，则此时最优解S*=V，且此时多项式时间内可求得最优解。
    ...
    //性质7：若图G=(V,E)为完全图，当(r+1)≤n时，V中任意(r+1)个结点组成的集合均为最优解。
    public static boolean property07(LinkedHashSet<Node> V) {
        for (Node node : V) {
